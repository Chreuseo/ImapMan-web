...
import { DataSourceTools, SavedListPreview } from "./data-source-tools";
...
export default async function Home({ searchParams }: PageProps<"/">) {
  ...
  if (selectedList?.list_type === "static") {
    try {
...
function Content({ page, mailboxes, distributors, lists, sources, messages, selectedList, members, mailboxAssignments, targetListAssignments, senderListAssignments }: { page: Page; mailboxes: Mailbox[]; distributors: Distributor[]; lists: MailingList[]; sources: DataSource[]; messages: ProcessedMessage[]; selectedList?: MailingList; members: Member[]; mailboxAssignments: Record<number, Distributor | null>; targetListAssignments: Record<number, MailingList[]>; senderListAssignments: Record<number, MailingList[]> }) {
  ...
  </form><MemberForm lists={lists} /></section><div><Table title="Vorhandene Listen" rows={lists} columns={["name", "list_type", "recipient_table"]} page="lists" path={(item) => `/lists/${item.id}`} /><section className="member-section"><h2>Gespeicherte Liste anzeigen</h2><form action="/" className="list-selector"><input type="hidden" name="page" value="lists" /><label>Liste auswählen<select defaultValue={selectedList?.id ?? ""} name="listId"><option value="">Auswählen</option>{lists.map((list) => <option value={list.id} key={list.id}>{list.name} ({list.list_type === "database" ? "Datenbank" : "statisch"})</option>)}</select></label><button type="submit">Anzeigen</button></form>{selectedList?.list_type === "static" && <MemberTable list={selectedList} members={members} />}{selectedList?.list_type === "database" && <SavedListPreview list={selectedList} />}{!selectedList && <p className="hint">Wählen Sie eine Liste aus.</p>}</section></div></div>;
...
function Table<T extends { id: number; name: string }>({ title, rows, columns, page, path }: { title: string; rows: T[]; columns: (keyof T)[]; page: string; path: (item: T) => string }) { return <section><h2>{title}</h2><table><thead><tr>{columns.map((column) => <th key={String(column)}>{String(column).replaceAll("_", " ")}</th>)}<th /></tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={row.id}>{columns.map((column) => <td key={String(column)}>{String(row[column] ?? "—")}</td>)}<td className="actions">{page === "lists" && "list_type" in row && <Link href={`/?page=lists&listId=${row.id}`}>{row.list_type === "static" ? "Mitglieder" : "Filter anzeigen"}</Link>}<DeleteButton path={path(row)} page={page} name={row.name} /></td></tr>) : <tr><td colSpan={columns.length + 1}>Keine Einträge vorhanden.</td></tr>}</tbody></table></section>; }
